                    ...
                    VStack {
                        ...
                        Map(coordinateRegion:
                                .constant(MKCoordinateRegion(center:
                                        .init(latitude: location.item.placemark.coordinate.latitude,
                                              longitude: location.item.placemark.coordinate.longitude),
                                                             span: .init(latitudeDelta: 0.0005,
                                                                         longitudeDelta: 0.0005))),
                            interactionModes: [],
                            annotationItems: [location],
                            annotationContent: { _ in
                            MapMarker(coordinate: .init(latitude: location.item.placemark.coordinate.latitude,
                                                        longitude: location.item.placemark.coordinate.longitude))
                        })
                        .frame(height: 150)
                        .cornerRadius(20)
